...
export async function storeProtocol(
  p: Pick<Protocol, "name" | "timeMin" | "timeSec" | "Zones" | "editorType">,
): Promise<number> {
  const db = await getDb();
  const zonesJson = JSON.stringify(p.Zones ?? {});

  const result = await db.runAsync(
    `INSERT INTO protocols (name, time_min, time_sec, zones_json, editor_type)
     VALUES (?, ?, ?, ?, ?);`,
    [p.name, p.timeMin, p.timeSec, zonesJson, p.editorType],
  );
...
export async function getProtocols(): Promise<Protocol[]> {
  ...
  const rows = await db.getAllAsync<{
    ...
    time_sec: number;
    zones_json: string;
    editor_type: "simple" | "complex";
  }>(
    `SELECT id, name, time_min, time_sec, zones_json, editor_type
     FROM protocols
     ORDER BY id ASC;`,
  );
...
export async function updateProtocol(
  ...
  await db.runAsync(
    `UPDATE protocols
     SET name = ?, time_min = ?, time_sec = ?, zones_json = ?, editor_type = ?
     WHERE id = ?;`,
    [p.name, p.timeMin, p.timeSec, zonesJson, p.editorType, id],
  );
}
